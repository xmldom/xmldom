...
;(function parseErrorLines() {
	let errorIndex = 0
	const source = fs
		.readFileSync(__dirname + '/../../lib/sax.js', 'utf8')
		.split('\n')
	source.forEach((lineFull, lineNumber) => {
		...
		if (/^(ParseError\.prototype|function ParseError)/.test(line)) {
			// ignoring ParseError "class" definition
			return
		}
		...
		LINE_TO_ERROR_INDEX[`lib/sax.js:${lineNumber + 1}`] = {
			errorType,
			index: errorIndex++,
			line,
			message,
	...
	Object.entries(REPORTED).forEach(([key, value]) => {
		...
		if (matches.length === 0)
			throw `${key} doesn't match any line in lib/sax.js`
		if (matches.length > 1) throw `${key} matches multiple lines in lib/sax.js`
		const lineKey = `lib/sax.js:${matches[0]}`
		if (!(lineKey in LINE_TO_ERROR_INDEX)) {
			console.error('line not mapped:', lineKey, matches[0])
		} else {
