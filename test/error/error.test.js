...
describe('errorHandle', () => {
  it("only single function with two args builds map", () => {
    ...
    var parser = new DOMParser({
		...
		assert(typeof error.warning === 'string', 'error.warning: '+String(error.warning));
		assert(typeof error.error === 'string', 'error.error: '+String(error.error));
  })
  ...
  it("only one function with one argument builds list", () => {
    ...
    var parser = new DOMParser({
		...
		assert(typeof error.warning === 'string', 'error.warning:'+error.warning);
		assert(typeof error.error === 'string', 'error.error:'+error.error);
  })
  ...
  it("error function throwing is not caught", () => {
	...
	var doc1 = parser.parseFromString('<html><body title="1<2"><table>&lt;;test</body></body></html>', 'text/html');
	try{
    	var doc2 = parser.parseFromString('<html><body title="1<2"><table&lt;;test</body></body></html>', 'text/html');
	}catch(e){
