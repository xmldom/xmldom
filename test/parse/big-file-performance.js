...
function xmldom(data) {
	console.time('xmldom')
	var doc = new DOMParser({ locator: null, checkLater: true }).parseFromString(
		data
	)
	...
	.addBatch({
		'big file parse': function () {
			...
			var data = fs.readFileSync(
				path.resolve(__dirname, './file-test1.xml'),
				'ascii'
			)
			...
			var domjsTime = t3 - t2
			console.assert(
				domjsTime > xmldomTime,
				'xmldom performance must more height!!'
			)
			...
			xmldomresult = (domjs(doc1 + '') + '').replace(
				/^<\?.*?\?>\s*|<!\[CDATA\[([\s\S]*?)\]\]>/g,
				xmlReplace
			)
			domjsresult = (doc2 + '').replace(
				/^<\?.*?\?>\s*|<!\[CDATA\[([\s\S]*?)\]\]>/g,
				xmlReplace
			)
			data = xmldomresult
