 ...
 * If it is not provided, the errors that do not stop parsing are reported to the console.
 * If provided, it is invoked for each error that occurs during parsing.
 * If the method provided throws any error, it rethrows it as a `ParserError`.
...
function DOMParser(options) {
	...
	if (typeof options.errorHandler === 'object') {
		throw new TypeError('errorHandler object is no longer supported, switch to onError!');
	} else if (options.errorHandler) {
		options.errorHandler('warning', 'The errorHandler option has been deprecated, use onError instead!');
	}
...
/**
 * Parses `source` using the options in the way configured by the `DOMParserOptions`
 * of this `DOMParser`. If `mimeType` is `text/html` an HTML `Document` is created,
 ...
 *   All error handlers can throw any kind of `Error`. The `fatalError` handler always throws a `ParseError`.
 * - All errors thrown during the parsing that are not a `ParseError`,
 *   are caught and reported using the `error` handler.
 * - If no `ParseError` is thrown, this method returns the `DOMHandler.doc`,
 *   which is the `Document` that has been created during parsing.
 ...
 * @returns {Document}
 * @throws ParseError for fatal errors, or errors thrown by `onError`
 *
...
DOMParser.prototype.parseFromString = function (source, mimeType) {
	...
	if (!domBuilder.doc) {
		sax.errorHandler.fatalError('no document was created');
	}
...
DOMHandler.prototype = {
	...
	reportError: function (level, message, defaultHandler) {
		if (typeof this.onError === 'function') {
			...
			(defaultHandler || console.error)('[xmldom ' + level + ']\t' + message, _locator(this.locator));
		}
	},
	/**
	...
	warning: function (message) {
		this.reportError('warning', message, console.warn);
	},
