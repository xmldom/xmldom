...
/**
 * Whether `prefix` is already declared in scope but bound to a namespace other than `uri`.
 * In that case the prefix cannot be reused without redeclaring it.
 *
 * @param {string} prefix
 * @param {string} uri
 * @param {Array.<{ prefix: string; namespace: string }>} visibleNamespaces
 * @returns {boolean}
 */
function isPrefixBoundToOtherNamespace(prefix, uri, visibleNamespaces) {
	var i = visibleNamespaces.length;
	while (i--) {
		var ns = visibleNamespaces[i];
		if (ns.prefix === prefix) {
...
/**
 * Generates a fresh namespace prefix (`ns1`, `ns2`, ...) that is not already declared in
 * scope,
 * following the "generate a prefix" step of the DOM Parsing and Serialization specification.
 *
 * @param {Array.<{ prefix: string; namespace: string }>} visibleNamespaces
 * @returns {string}
 * @see https://w3c.github.io/DOM-Parsing/#dfn-generating-a-prefix
 */
function generateNamespacePrefix(visibleNamespaces) {
	var counter = 1;
	var candidate;
	do {
		candidate = 'ns' + counter++;
	} while (isPrefixInUse(candidate, visibleNamespaces));
	return candidate;
}
/**
 * @param {string} prefix
 ...
 * @returns {boolean}
 */
function isPrefixInUse(prefix, visibleNamespaces) {
	var i = visibleNamespaces.length;
	while (i--) {
		if (visibleNamespaces[i].prefix === prefix) {
			return true;
		}
	}
	return false;
					...
					case ELEMENT_NODE:
						...
						for (var i = 0; i < len; i++) {
							...
							if (needNamespaceDefine(attr, isHTML, childNamespaces)) {
								...
								// If the requested prefix is already bound to a different namespace in scope,
								// reusing it would emit a second `xmlns:<prefix>` and break the
								// "Unique Att Spec" well-formedness constraint (XML 1.0 5th ed. 3.1).
								// Following the DOM-Parsing "generate a prefix" step, pick a fresh `ns<N>`.
								if (attrPrefix && isPrefixBoundToOtherNamespace(attrPrefix, uri, childNamespaces)) {
									attrPrefix = generateNamespacePrefix(childNamespaces);
									generatedName = attrPrefix + ':' + attr.localName;
								}
							...
							if (filteredAttr) {
								if (typeof filteredAttr === 'string') {
									...
									addSerializedAttribute(buf, generatedName || filteredAttr.name, filteredAttr.value, requireWellFormed);
								}
							}
						}
