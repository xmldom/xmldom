...
/**
 * Returns a live NodeList containing all the elements with the specified class name.
 *
 * @param {string} className
 * The class name to search for.
 * @returns {Node[]}
 * A live NodeList containing all the elements with the specified class name.
 */
LiveNodeList.prototype.getElementsByClassName = function (className) {
	// Create an empty array to store the matching elements
	var result = [];
	// Iterate through each node in the LiveNodeList
	for (var i = 0; i < this.length; i++) {
		// Use the native getElementsByClassName method to get the matching elements
		var elements = this[i].getElementsByClassName(className);
		// Add the matching elements to the result array
...
Element.prototype = {
	...
	/**
	 * Returns a HTMLCollection of elements with the given class name.
	 * The complete document is searched, including the root node.
	 * The returned HTMLCollection is live, meaning that it updates itself automatically to stay
	 * in sync with the DOM treewithout having to call `document.getElementsByClassName()` again.
	 *
	 * @param {string} classNames
