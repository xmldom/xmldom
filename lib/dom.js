...
var INTERNAL_SYMBOL = conventions.INTERNAL_SYMBOL;
...
DOMImplementation.prototype = {
	...
	createDocument: function (namespaceURI, qualifiedName, doctype) {
		...
		var doc = new Document({ contentType: contentType }, INTERNAL_SYMBOL);
	...
	createDocumentType: function (qualifiedName, publicId, systemId, internalSubset) {
		validateQualifiedName(qualifiedName);
		var node = new DocumentType(INTERNAL_SYMBOL);
	...
	createHTMLDocument: function (title) {
		var doc = new Document({ contentType: MIME_TYPE.HTML }, INTERNAL_SYMBOL);
		doc.implementation = this;
		doc.childNodes = new NodeList();
		if (title !== false) {
 ...
 */
function Document(options, symbol) {
	if (symbol !== INTERNAL_SYMBOL) {
		throw new Error('Illegal constructor');
	}
...
Document.prototype = {
	...
	createElement: function (tagName) {
		var node = new Element(INTERNAL_SYMBOL);
	...
	createDocumentFragment: function () {
		var node = new DocumentFragment(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.childNodes = new NodeList();
		return node;
	},
	createTextNode: function (data) {
		var node = new Text(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.appendData(data);
		return node;
	},
	createComment: function (data) {
		var node = new Comment(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.appendData(data);
		return node;
	},
	createCDATASection: function (data) {
		var node = new CDATASection(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.appendData(data);
		return node;
	},
	createProcessingInstruction: function (target, data) {
		var node = new ProcessingInstruction(INTERNAL_SYMBOL);
		node.ownerDocument = this;
	...
	_createAttribute: function (name) {
		var node = new Attr(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.name = name;
		node.nodeName = name;
		node.localName = name;
	...
	createEntityReference: function (name) {
		var node = new EntityReference(INTERNAL_SYMBOL);
		node.ownerDocument = this;
		node.nodeName = name;
		return node;
	},
