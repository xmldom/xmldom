 ...
 */
function serializableNamespacePrefix(node, visibleNamespaces) {
	if (node.namespaceURI === NAMESPACE.XMLNS) return node.prefix;
	if (node.namespaceURI === '' || node.namespaceURI === null) return null;
	var prefix = node.prefix || '';
	if (prefix !== '') return node.prefix;
	...
	var GENERATED_PREFIX = 'ns';
	var GENERATED_PREFIX_REG = /ns\d+/;
	// find the maximum number already used in all visible namespaces, use the next one
	var nextNumber =
		visibleNamespaces.reduce(function (next, ns) {
			if (!ns.prefix || !GENERATED_PREFIX_REG.test(ns.prefix)) return next;
			return Math.max(next, parseInt(ns.prefix.replace(GENERATED_PREFIX, '')));
		}, 0) + 1;
	return GENERATED_PREFIX + nextNumber;
}
	...
	switch (node.nodeType) {
		case ELEMENT_NODE:
			...
			for (var i = 0; i < len; i++) {
				...
				var assumedPrefix = serializableNamespacePrefix(attr, visibleNamespaces);
...
exports._serializableNamespacePrefix = serializableNamespacePrefix;
exports.Attr = Attr;
