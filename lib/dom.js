...
function serializeToString(node, buf, nodeFilter, visibleNamespaces) {
	if(buf.length > 0 && buf.length % 10000000 == 0){
		console.log('serializeToString Buf:', buf.length)
	}
			...
			if (canCloseTag) {
				...
				if (isHTML && isHTMLRawTextElement(nodeName)) {
					...
					while (child) {
						...
						var newBuf = []
						serializeToString(child, newBuf, nodeFilter, visibleNamespaces.slice());
						buf.push(newBuf.join(''))
						child = child.nextSibling;
					}
				}
				buf.push('</', prefixedNodeName, '>');
