...
function assumePrefix(node, visibleNamespaces) {
	...
	if (uri === '' || uri == null) {
		// I don't think we should be doing new validation here, but if we wanted to the following
		// makes sense:
	...
	// Don't assume prefixes for non-attributes or for attributes that already
	// have a prefix.
	if (nodeType !== ATTRIBUTE_NODE || prefix !== '') return node.prefix;
	...
	switch (node.nodeType) {
		case ELEMENT_NODE:
			...
			for (var i = 0; i < len; i++) {
				...
				if (attr.namespaceURI != null && (attr.prefix == '' || attr.prefix == null)) {
