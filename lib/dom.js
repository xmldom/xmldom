 ...
 * @todo This function does not fully implement the validation critera from the WHATWG DOM
 *       spec.
 */
...
// TODO: These are redundant, we should use types on Node instead, eg: `Node.ELEMENT_NODE`
var NodeType = {};
...
// TODO: These are redundant, we should use codes on `DOMException` directly, eg: `DOMException.INDEX_SIZE_ERR`
var ExceptionCode = {};
var INDEX_SIZE_ERR = (ExceptionCode.INDEX_SIZE_ERR = 1);
var DOMSTRING_SIZE_ERR = (ExceptionCode.DOMSTRING_SIZE_ERR = 2);
var HIERARCHY_REQUEST_ERR = (ExceptionCode.HIERARCHY_REQUEST_ERR = 3);
var WRONG_DOCUMENT_ERR = (ExceptionCode.WRONG_DOCUMENT_ERR = 4);
var INVALID_CHARACTER_ERR = (ExceptionCode.INVALID_CHARACTER_ERR = 5);
var NO_DATA_ALLOWED_ERR = (ExceptionCode.NO_DATA_ALLOWED_ERR = 6);
var NO_MODIFICATION_ALLOWED_ERR = (ExceptionCode.NO_MODIFICATION_ALLOWED_ERR = 7);
var NOT_FOUND_ERR = (ExceptionCode.NOT_FOUND_ERR = 8);
var NOT_SUPPORTED_ERR = (ExceptionCode.NOT_SUPPORTED_ERR = 9);
var INUSE_ATTRIBUTE_ERR = (ExceptionCode.INUSE_ATTRIBUTE_ERR = 10);
var INVALID_STATE_ERR = (ExceptionCode.INVALID_STATE_ERR = 11);
var SYNTAX_ERR = (ExceptionCode.SYNTAX_ERR = 12);
var INVALID_MODIFICATION_ERR = (ExceptionCode.INVALID_MODIFICATION_ERR = 13);
var NAMESPACE_ERR = (ExceptionCode.NAMESPACE_ERR = 14);
var INVALID_ACCESS_ERR = (ExceptionCode.INVALID_ACCESS_ERR = 15);
var VALIDATION_ERR = (ExceptionCode.VALIDATION_ERR = 16);
var TYPE_MISMATCH_ERR = (ExceptionCode.TYPE_MISMATCH_ERR = 17);

// TODO: These are redundant, we should use positions on `Node` directly, eg: `Node.DOCUMENT_POSITION_DISCONNECTED`
 ...
 * @todo This does not conform to the living standard:
 *       https://webidl.spec.whatwg.org/#idl-DOMException The living standard version accepts a
 *       `name` parameter instead of the legacy `code`:
...
/* TODO: this is potentially a bug, eg:
DOMException.prototype = Error.prototype;
DOMException.prototype.foo = 'bar';
var e = new Error();
e.foo === 'bar'; // true
Should use _extends instead.
 */
DOMException.prototype = Error.prototype;
/**
...
// TODO: We should make these conform to the error names table: https://webidl.spec.whatwg.org/#dfn-error-names-table
 ...
 * @constructs NodeList
 * @todo Properly implement NodeFilter
 ...
 * @todo The spec states that it is used only for Attr, but the doc says its for DocumentType
 *       as well?
 */
...
DOMImplementation.prototype = {
	 ...
	 *      Level 3 Core
	 * @todo - There is no `internalSubset` parameter in the specs - Validate qualifiedName using
	 *       grammar.
	 */
 ...
 * @todo Undeclared/missing properties and methods: nodeType, baseURI, isConnected,
 *       getRootNode(),
 *       parentElement, textContent, isEqualNode(), isSameNode(), contains()
 */
function Node() {}
Node = {
	/**
	 ...
	 * @type {number}
	 * @constant
	 * @see {@link ProcessingInstruction}
	 * @static
	 * @default 7
	 */
	...
	/**
	 * `otherNode` precedes the node in either a
	 * {@link https://en.wikipedia.org/wiki/Tree_traversal#Pre-order,_NLR pre-order depth-first
	 * traversal} of a tree containing both (e.g., as an ancestor or previous sibling or a
	 * descendant of a previous sibling or previous sibling of an ancestor) or (if they are
	 * disconnected) in an arbitrary but consistent ordering.
	 *
	 * @type {number}
	 * @constant
	 * @see {@link https://dom.spec.whatwg.org/#dom-node-comparedocumentposition}
	...
	/**
	 * `otherNode` follows the node in either a
	 * {@link https://en.wikipedia.org/wiki/Tree_traversal#Pre-order,_NLR pre-order depth-first
	 * traversal} of a tree containing both (e.g., as a descendant or next sibling or a descendant
	 * of a next sibling) or (if they are disconnected) in an arbitrary but consistent ordering.
	 *
	 * @type {number}
	 * @constant
	 * @see {@link https://dom.spec.whatwg.org/#dom-node-comparedocumentposition}
...
Node.prototype = {
	 ...
	 *
	 * _Note: The implementation appears incomplete when compared to the algorithm described in
	 * the specification. Specifically, it does not incorporate the specific branching logic based
	 * on different node interfaces as outlined in the spec (Element, Document,
	 * DocumentType, DocumentFragment, Attr, etc.)_.
	 *
	 * @param {string | null} namespaceURI
	 ...
	 * @todo Implement the algorithm as described in the WHATWG specification.
	 */
	 ...
	 *
	 * _Note, this implementation differs from the WHATWG specification._.
	 *
