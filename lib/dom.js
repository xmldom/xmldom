 ...
 * content that would produce ill-formed XML (e.g. Text data containing characters outside the
 * XML Char production, or a Document with no `documentElement`).
 *
 ...
 * next breaking release. Callers should migrate to `{ requireWellFormed: true }`.
 *
 ...
 * @returns {string}
 * @throws {DOMException}
 * With name `InvalidStateError` when `requireWellFormed` is `true` and Text data contains
 * characters outside the XML Char production, or the Document has no `documentElement`.
	...
	switch (node.nodeType) {
		case ELEMENT_NODE:
			...
			for (var i = 0; i < len; i++) {
				...
				serializeToString(attr, buf, visibleNamespaces, opts);
			}
				...
				if (isHTML && isHTMLRawTextElement(nodeName)) {
					while (child) {
						if (child.data) {
							...
							serializeToString(child, buf, visibleNamespaces.slice(), opts);
						}
						child = child.nextSibling;
					}
				} else {
		...
		case DOCUMENT_FRAGMENT_NODE:
			if (requireWellFormed && node.nodeType === DOCUMENT_NODE && node.documentElement == null) {
				throw new DOMException('The Document has no documentElement', 'InvalidStateError');
			}
			var child = node.firstChild;
			while (child) {
				serializeToString(child, buf, visibleNamespaces.slice(), opts);
		...
		case TEXT_NODE:
			 ...
			 */
			if (requireWellFormed && g.InvalidChar.test(node.data)) {
				throw new DOMException('The Text node data contains characters outside the XML Char production', 'InvalidStateError');
			}
