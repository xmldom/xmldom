...
Node.prototype = {
	 ...
	 */
	normalize: function () {
		// Use iterative approach with a stack to prevent stack overflow on deeply nested structures
		var stack = [this];
		...
		while (stack.length > 0) {
			...
			while (child) {
				...
				if (next && next.nodeType == TEXT_NODE && child.nodeType == TEXT_NODE) {
					...
					// Don't advance child, check if there are more adjacent text nodes
				} else {
					// Add child to stack for processing if it has children
					if (child.firstChild) {
						stack.push(child);
					}
					child = next;
...
function serializeToString(node, buf, nodeFilter, visibleNamespaces) {
	...
	switch (node.nodeType) {
		...
		case COMMENT_NODE:
			// Prevent --> injection by replacing -- with -&#x2D;
			// This prevents any -- sequence which could form --> and close the comment prematurely
			// Per XML spec, comments cannot contain -- except in the closing delimiter
			return buf.push(g.COMMENT_START, node.data.replace(/--/g, '-&#x2D;'), g.COMMENT_END);
		...
		case PROCESSING_INSTRUCTION_NODE:
			// Prevent ?> injection by replacing any occurrence with ?&#x3E;
			// This prevents premature PI closure while maintaining valid XML
			return buf.push('<?', node.target, ' ', node.data.replace(/\?>/g, '?&#x3E;'), '?>');
