...
/**
 * Finds an existing prefix for a namespace URI in the visible namespaces.
 *
 * @param {string} namespaceURI
 * - The namespace URI to find a prefix for.
 * @param {Array} visibleNamespaces
 * - Array of visible namespace mappings.
 * @returns {string | null}
 * The existing prefix or null if not found.
 */
function findExistingPrefix(namespaceURI, visibleNamespaces) {
	for (var i = visibleNamespaces.length - 1; i >= 0; i--) {
		var ns = visibleNamespaces[i];
		if (ns.namespace === namespaceURI && ns.prefix && ns.prefix !== '') {
			return ns.prefix;
		}
	}
	return null;
...
/**
 * Checks if a prefix conflicts with existing namespace bindings.
 *
 * @param {string} prefix
 * - The prefix to check.
 * @param {string} namespaceURI
 ...
 */
function hasPrefixConflict(prefix, namespaceURI, visibleNamespaces, explicitPrefixes) {
	for (var i = 0; i < visibleNamespaces.length; i++) {
		var ns = visibleNamespaces[i];
		if (ns.prefix === prefix && ns.namespace !== namespaceURI) {
			return true;
		}
	}
	if (explicitPrefixes) {
		// Only used for generated prefixes
		for (var j = 0; j < explicitPrefixes.length; j++) {
			if (explicitPrefixes[j] === prefix) {
				return true;
			}
		}
	}
...
/**
 * Generates a new unique prefix that doesn't conflict with existing bindings.
 *
 * @param {string} namespaceURI
 * - The namespace URI to generate a prefix for.
 * @param {Array} visibleNamespaces
 * - Array of visible namespace mappings.
 * @param {Object} prefixIndex
 * - Object with 'value' property tracking the next prefix number.
 * @returns {string}
 * A unique prefix.
 */
function generateUniquePrefix(namespaceURI, visibleNamespaces, prefixIndex, usedPrefixes) {
	var generatedPrefix;
	do {
		generatedPrefix = 'ns' + prefixIndex.value;
		prefixIndex.value++;
	} while (hasPrefixConflict(generatedPrefix, namespaceURI, visibleNamespaces, usedPrefixes));
...
function serializeToString(node, buf, nodeFilter, visibleNamespaces, prefixIndex) {
	...
	switch (node.nodeType) {
		case ELEMENT_NODE:
			...
			// Pass 1: collect explicit xmlns declarations and explicitly specified prefixes
			var explicitPrefixes = [];
			for (var i = 0; i < attrsLen; i++) {
				var attr = attrs.item(i);
				if (attr.prefix === 'xmlns') {
					// prefixed xmlns declarations
					visibleNamespaces.push({
						prefix: attr.localName,
						namespace: attr.value,
					});
					explicitPrefixes.push(attr.localName);
				} else if (attr.nodeName === 'xmlns') {
					// default xmlns declaration
					visibleNamespaces.push({ prefix: '', namespace: attr.value });
				} else if (attr.prefix) {
					// track the prefixes attributes use to avoid conflicts with generated prefixes
					explicitPrefixes.push(attr.prefix);
				}
			}

			// Pass 2: define namespaces if needed and serialize attributes
			for (var i = 0; i < attrsLen; i++) {
				var attr = attrs.item(i);
				if (needNamespaceDefine(attr, isHTML, visibleNamespaces)) {
					// Even if a prefix is explicitly specified, we must check if it conflicts with some other namespace
					if (attr.prefix && !hasPrefixConflict(attr.prefix, attr.namespaceURI, visibleNamespaces, null)) {
						addSerializedAttribute(buf, 'xmlns:' + attr.prefix, attr.namespaceURI);
						visibleNamespaces.push({ prefix: attr.prefix, namespace: attr.namespaceURI });
					} else {
						attr._generatedPrefix = findExistingPrefix(attr.namespaceURI, visibleNamespaces);
						if (!attr._generatedPrefix) {
							attr._generatedPrefix = generateUniquePrefix(attr.namespaceURI, visibleNamespaces, prefixIndex, explicitPrefixes);
							addSerializedAttribute(buf, 'xmlns:' + attr._generatedPrefix, attr.namespaceURI);
							visibleNamespaces.push({ prefix: attr._generatedPrefix, namespace: attr.namespaceURI });
						}
					}
				}
				serializeToString(attrs.item(i), buf, nodeFilter, visibleNamespaces, prefixIndex);
			}
			...
			if (canCloseTag) {
				...
				if (isHTML && isHTMLRawTextElement(nodeName)) {
					...
					while (child) {
						serializeToString(child, buf, nodeFilter, visibleNamespaces.slice(), prefixIndex);
						child = child.nextSibling;
					}
				}
				buf.push('</', prefixedNodeName, '>');
		...
		case ATTRIBUTE_NODE:
			...
			// Use generated prefix if available
			if (node._generatedPrefix) {
				attrName = node._generatedPrefix + ':' + node.localName;
			}
			return addSerializedAttribute(buf, attrName, node.value);
		case TEXT_NODE:
			/*
