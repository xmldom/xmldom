...
/**
 * Updates `el.childNodes`, `el.firstElementChild` and `el.lastElementChild` and also updates
 * the `previousElementSibling` and `nextElementSibling` links of child elements.
	...
	if (doc && doc._inc) {
		...
		if (newChild) {
			...
			if (isElementNode(newChild)) {
				...
				if (!el.firstElementChild) el.firstElementChild = newChild;
				newChild.previousElementSibling = el.lastElementChild;
				if (el.lastElementChild) {
					el.lastElementChild.nextElementSibling = newChild;
				}
				el.lastElementChild = newChild;
			}
		} else {
			el.childElementCount = 0;
			el.firstElementChild = el.lastElementChild = null;
			...
			while (child) {
				...
				if (isElementNode(child)) {
					...
					if (el.lastElementChild) {
						el.lastElementChild.nextElementSibling = child;
					}
					child.previousElementSibling = el.lastElementChild;
...
function _insertBefore(parent, node, child, _inDocumentAssertion) {
	...
	_onUpdateChild(parent.ownerDocument || parent, parent, child == null && node.nodeType !== DOCUMENT_FRAGMENT_NODE ? node : null);
