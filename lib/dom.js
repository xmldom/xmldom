...
NamedNodeMap.prototype = {
	 ...
	 * - this method differs from setNamedItemNS
	 *   by using getNamedItem instead of getNamedItemNS,
	 *   meaning it resolves items case-insensitive in HTML documents
	 *   (fixing it causes several tests to fail, some with duplicate attributes)
	 *
	 * @param {Attr} attr
	 * @return {Attr | null}
	...
	setNamedItem: function (attr) {
		...
		// implementation according to the spec(?) causing duplicate attrs in tests:
		// using this drops case-insensitive HTML node resolution,
		// so maybe some code is using this method in a wrong way
		// which should take care of handling the case itself.
		// aligning this method one of the methods can call the other one
		// instead of reimplementing it!
		// var oldAttr = this.getNamedItemNS(attr.namespaceURI, attr.nodeName);
...
Element.prototype = {
	 ...
	 *
	 * __This implementation differs from the specification:__
	 * - it is not validating namespace or qualified name (step 1 of the spec), so passing a qualified name as the second argument
	 *   will add a duplicate attribute.
	 *
	 * @param {string} namespaceURI
	 * @param {string} localName
	 * @param {string} value
