...
var Char = /[-\x09\x0A\x0D\x20-\x2C\x2E-\uD7FF\uE000-\uFFFD]/; // | [#x10000-#x10FFFF]
...
var markupdecl_uv = regg(elementdecl, '|', AttlistDecl, '|', EntityDecl, '|', NotationDecl, '|', PI_unsafe, '|', Comment);
...
var intSubset_uv = reg(regg(markupdecl_uv, '|', PEReference, '|', S), '*');

// https://www.w3.org/TR/xml11/#NT-doctypedecl
// [28] doctypedecl ::=          '<!DOCTYPE' S Name (S ExternalID)? S? ('[' intSubset ']' S?)? '>'
// https://www.w3.org/TR/xml-names/#NT-doctypedecl
// [16] doctypedecl ::=          '<!DOCTYPE' S QName (S ExternalID)? S? ('[' (markupdecl | PEReference | S)* ']' S?)? '>'
var doctypedecl = reg('<!DOCTYPE', S, Name, regg(S, ExternalID), '?', S_OPT, regg(/\[/, intSubset_uv, /]/, S_OPT), '?', '>');
