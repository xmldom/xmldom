...
function chars_without(regexp, search) {
	...
	if (!search || typeof search !== 'string') {
		throw new Error(JSON.stringify(search) + ' is not a valis search');
	}
	...
	var result = new RegExp(regexp.source.replace(search, ''), UNICODE_SUPPORT ? 'u' : '');
...
function reg_chars(args) {
	var result = reg.apply(reg_chars, Array.prototype.slice.call(arguments));
	result.chars = chars(result);
	return result;
}
...
if (UNICODE_SUPPORT) {
	...
	Char = reg_chars('[', chars(Char), '\\u{10000}-\\u{10FFFF}', ']');
}

var _SChar = /[\x20\x09\x0D\x0A]/;
// https://www.w3.org/TR/xml11/#NT-S
// [3] S ::= (#x20 | #x9 | #xD | #xA)+
var S = reg_chars(_SChar, '+');
...
var S_OPT = reg_chars(_SChar, '*');

// https://www.w3.org/TR/xml11/#NT-NameStartChar
// [4] NameStartChar ::= ":" | [A-Z] | "_" | [a-z] | [#xC0-#xD6] | [#xD8-#xF6] | [#xF8-#x2FF] | [#x370-#x37D] | [#x37F-#x1FFF] | [#x200C-#x200D] | [#x2070-#x218F] | [#x2C00-#x2FEF] | [#x3001-#xD7FF] | [#xF900-#xFDCF] | [#xFDF0-#xFFFD] | [#x10000-#xEFFFF]
var NameStartChar =
	/[:A-Z_a-z\xC0-\xD6\xD8-\xF6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD]/; // without \u10000-\uEFFFF
NameStartChar.chars = chars(NameStartChar);
if (UNICODE_SUPPORT) {
	// eslint-disable-next-line es5/no-unicode-code-point-escape
	NameStartChar = reg_chars('[', NameStartChar.chars, '\\u{10000}-\\u{10FFFF}', ']');
}
...
var PI_match = reg(/^<\?/, '(', Name, ')', regg(S, '(', Char, '*?)'), '?', /\?>/);
...
var Comment = reg('<!--', regg(chars_without(Char, '-'), '|', reg('-', chars_without(Char, '-'))), '*', '-->');
...
// var _choice_or_seq = reg('[', NameChar.chars, S.chars, chars(_children_quantity), '()|,]*');
...
// simplification to solve circular referencing, but doesn't check validity constraint "Proper Group/PE Nesting"
var children = reg(/[^>]+/ /*regg(choice, '|', seq), _children_quantity*/);
...
// they also need to support PEReference when checked before replacement of entities declared in DTD (which xmldom is not supporting)
...
var AttlistDecl = reg('<!ATTLIST', S, Name, AttDef, '*', S_OPT, '>');
...
var GEDecl = reg('<!ENTITY', S, Name, S, EntityDef, S_OPT, '>');
// [74] PEDef ::= EntityValue | ExternalID
var PEDef = regg(EntityValue, '|', ExternalID);
// [72] PEDecl ::= '<!ENTITY' S '%' S Name S PEDef S? '>'
var PEDecl = reg('<!ENTITY', S, '%', S, Name, S, PEDef, S_OPT, '>');
...
// [26]   	VersionNum	   ::=   	'1.' [0-9]+
var VersionNum = reg('1.', /\d/, '+');
// [24] VersionInfo	::= S 'version' Eq ("'" VersionNum "'" | '"' VersionNum '"')
var VersionInfo = reg(S, 'version', Eq, regg("'", VersionNum, "'", '|', '"', VersionNum, '"'));
// [81]   	EncName	   ::=   	[A-Za-z] ([A-Za-z0-9._] | '-')*
var EncName = reg(/[A-Za-z]/, /[-A-Za-z0-9._]/, '*');
