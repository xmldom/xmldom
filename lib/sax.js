...
/**
 * @typedef ParseUtils
 * @property {function(number?): string} char
 * @property {function(): number} getIndex
 * @property {function(RegExp): string} getMatch
 * @property {function(): string} getSource
 * @property {function(number?): void} skip
 * @property {function(): number} skipBlanks
 * @property {function(): string} substringFromIndex
 * @property {function(string): boolean} substringStartsWith
 */
...
function parseUtils(source, start) {
	...
	function getMatch(args) {
		var expr = g.reg('^(', args, ')');
		var match = substringFromIndex().match(expr);
		if (match) {
			skip(match[1].length);
			return match[1];
		}
...
function parseDoctypeInternalSubset(p, errorHandler) {
	...
	function parsePI(p, errorHandler) {
		var match = p.substringFromIndex().match(g.PI);
		if (!match) {
