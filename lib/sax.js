	...
	switch (next) {
		...
		default:
			...
			if (len > 1 && /!doctype/i.test(matchs[0][0])) {
				...
				var doctype = extractDoctype(source);
...
function extractDoctype(source, start) {
	start = start || 0;
	var end = start;
	var foundDoctype = null;
	var bracketLevel = 0;
	var currentQuote = null;
	var inComment = false;
	for (var i = start; i < source.length; i++) {
		if (!inComment && currentQuote === null) {
		...
		if (inComment) {
			if (source.substring(i, i + 3) === '-->') {
				inComment = false;
				i += 2;
			}
			continue;
		}
		if ((source[i] === '"' || source[i] === "'") && (currentQuote == null || source[i - 1] !== '\\')) {
			currentQuote = currentQuote === null ? source[i] : currentQuote === source[i] ? null : currentQuote;
			continue;
		}
		if (currentQuote !== null) continue;
		if (bracketLevel === 0 && source[i] === '<' && source[i + 1] === '!') {
			var tmpStart = i;
			i = skipWhitespace(source, i + 2);
			if (source.substring(i, i + 7).toLowerCase() === 'doctype') {
				bracketLevel++;
				foundDoctype = tmpStart;
			}
		} else {
			if (source[i] === '<') bracketLevel++;
			else if (source[i] === '>' && --bracketLevel === 0) {
	...
	return source.substring(foundDoctype, end + 1);
}
